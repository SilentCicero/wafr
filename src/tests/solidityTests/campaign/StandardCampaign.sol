...
contract StandardCampaign is Campaign {
  ...
  function contributeMsgValue() payable atStageOr(uint256(Stages.CrowdfundOperational)) public returns (uint256 contributionID) {
  ...
  function payoutToBeneficiary() atStageOr(uint256(Stages.CrowdfundSuccess)) public returns (uint256 amountClaimed) {
  ...
  function claimRefundOwed(uint256 _contributionID) atStageOr(uint256(Stages.CrowdfundFailure)) validRefundClaim(_contributionID) public returns (address balanceClaim) {
